...
include_once("db-connect.php"); //include config file
...
if(isset($_POST["type"]) && $_POST["type"]=='add' && $_POST["product_qty"]>0)
{
    ...
    $statement = $db->prepare("SELECT Name, Price FROM products WHERE id = :id LIMIT 1");
    $statement->bindParam(':id', $new_product['product_code']);
    $statement->execute();
    //$statement->bind_result($product_name, $price);
    echo 'almost';
    echo $new_product['product_code'];
    while($row = $statement->fetch(PDO::FETCH_ASSOC)){
        //fetch product name, price from db and add to new_product array
        $new_product["product_name"] = $row['Name']; 
